...
class User extends BaseController
{
    ...
    public function login()
    {
        ...
        if ($user) {
            ...
            if ($access) {
                ...
                return jok('登录成功', ['access_token' => $access['access_token'], 'user' => $user]);
            } else {
                return jerr('登录系统异常');
            }
        } else {
     ...
     * 用户注册，没有code，包含单位
     */
    public function registerWithNoCode() {
        if (!input("phone")) {
            return jerr("手机号不能为空！", 400);
        }
        $phone = input("phone");
        ...
        if ($password !== $password_confirm) {
            return jerr('两次密码不一致', 400);
        }
        if (!input("unit")) {
            return jerr("单位不能为空！", 400);
        }
        $name = $phone;
        $unit = input("unit");
        if (!input("true_name")) {
            return jerr("姓名不能为空！", 400);
        }
        ...
        $result = $this->model->reg($phone, $password, $name, $unit, $true_name);
        if ($result) {
            return jok("用户注册成功");
        } else {
            return jerr("注册失败，请重试！");
