...
h = (function() {
    ...
    function beat() {
        ...
        document.getElementById('inActiveTime')!.innerText = inActiveTime.toString()
    }
...
if ('onfocusin' in document) {
    (document as any).onfocusin = onFocus;
    (document as any).onfocusout = onBlur
} else {
...
if (isMobile()) {
    ...
    if (eventKey) {
        addEvent(document, eventKey, function() {
            (document as any)[hiddenKey] ? onBlur() : onFocus()
        })
    }
}
inActiveTime = 0
...
function addEvent(element: any, event: string, callback: any) {
