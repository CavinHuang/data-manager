 ...
 * @param item
 */
export function formItemConvertData (item: any, projectKey: any) {
  const data = {
    dId: item.dId,
    type: item.__config__.typeId,
    ...
    questionKey: projectKey
  }
  const expand = {}
  const param = dataParams[item.__config__.typeId]
...
export function dbDataConvertForItemJson (data: any) {
  ...
  if (+typeMap.size <= 0) {
    // 根据类型获取默认数据
    const _selectComponents: any = selectComponents
    _.concat(inputComponents, _selectComponents, imageComponents, assistComponents, personalInfoComponents).forEach(item => {
      typeMap.set(item.__config__.typeId, item)
    })
  }
  ...
  if (data.defaultValue) {
    if (data.defaultValue.json) {
      jsonItem.__config__.defaultValue = data.defaultValue.json
    } else {
